...
let InvalidIteratorError = require('./invaliditeratorerror.js');
...
class Aligner {
        ...
        switch (dataSource.sourceDescription.type) {
          ...
          case 'json':
            if (dataSource.sourceDescription.iterator) {
              ...
              let result = Aligner._alignValuesWithJSONDataSourceAndSearchIterator(dataSource, values);
  ...
  _createJoinConditions(semanticmodel) {
    ...
    edges.forEach(edge => {
      ...
      if (sourceNode.sourceDescription && targetNode.sourceDescription) {
        ...
        if (sourceData.sourceDescription.type === 'csv' && targetData.sourceDescription.type === 'csv') {
          let sourceRow = sourceData.row;
          let targetRow = targetData.row;
          let match = undefined;
          let i = 0;

          while (i < sourceRow.length && !match) {
            let j = 0;

            while (j < targetRow.length && sourceRow[i].value !== targetRow[j].value) {
              j++;
            }

            if (j < targetRow.length) {
              match = {
                child: sourceRow[i].column,
                parent: targetRow[j].column
              };
            } else {
              i++;
            }
          ...
          if (match) {
            //console.log(`join condition: child = ${match.child} and parent = ${match.parent}`);
  ...
  static _alignValuesWithJSONDataSourceAndSearchIterator(dataSource, values) {
    //console.log(dataSource.object);
    //console.log(values);
    return pinta.iteratorAndPaths(dataSource.object, values);
  }
}

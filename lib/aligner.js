...
class Aligner {
  ...
  _alignWithAllDataSources(semanticmodel) {
    ...
    classNodes.forEach(classNode => {
      ...
      if (!N3.Util.isBlank(classNode.sample.value)) {
        let hashPosition = classNode.sample.value.lastIndexOf('#');
        let slashPosition = classNode.sample.value.lastIndexOf('/');
        position = hashPosition > slashPosition ? hashPosition : slashPosition;
        classNodeValue = classNode.sample.value.substring(position + 1);
        //console.log(`classNodeValue = ${classNodeValue}`);

        values.push(classNodeValue);
      } else {
        classNode.isBlankNode = true;
      }
      ...
      this.dataSources.forEach(function (dataSource) {
        ...
        if (valuesAndPaths) {
          valuesAndPaths.forEach(function (vp) {
            //console.log(vp)
            if (classNodeValue !== undefined && classNodeValue === vp.value) {
              //console.log('t');
   ...
   * @param setDatatypeAndLanguage: if true the datatypes and languages are taken into account
   */
  align(semanticmodel, findJoinConditions = true, setDatatypeAndLanguage = true) {
    this._alignWithAllDataSources(semanticmodel);
    this._selectSingleDataSourceForEachNode(semanticmodel);

    if (setDatatypeAndLanguage) {
      Aligner._setLanguageAndDataType(semanticmodel);
    }
  ...
  static _setLanguageAndDataType(semanticmodel) {
    let nodes = semanticmodel.getAllNodes(type.DATAREFERENCE);

    nodes.forEach(node => {
      if (node.sourceDescription) {
        node.language = node.sample.language;
        node.datatype = node.sample.type;
      }
    });
  }
}
